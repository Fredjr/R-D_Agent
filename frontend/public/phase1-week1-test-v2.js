/**
 * Phase 1 Week 1 - Browser-Based E2E Testing Script (VERSION 2 - FIXED FILTERS)
 *
 ...
 *
 * FIXES IN V2:
 * - Fixed filter tests to look for any UI element (not just <select>)
 * - More flexible text matching for filter options
 * - Better handling of custom dropdown components
 */
...
(async function testPhase1Week1() {
  ...
  log('\n╔═══════════════════════════════════════════════════════════╗', colors.blue);
  log('║      PHASE 1 WEEK 1 - BROWSER E2E TESTS V2               ║', colors.blue);
  log('╚═══════════════════════════════════════════════════════════╝\n', colors.blue);
  ...
  logTest('2.2 Quick stats cards are present', statsCards.length >= 4);
  ...
  if (filterButton) {
    ...
    // Test 5.5: Type filter exists (FIXED V2)
    const typeFilterUI = Array.from(document.querySelectorAll('*')).find(el => {
      const text = el.textContent?.toLowerCase() || '';
      const hasTypeLabel = text.includes('type') || text.includes('note type');
      const hasTypeOptions = text.includes('general') || text.includes('finding') || text.includes('question');
      return hasTypeLabel && hasTypeOptions && el.children.length > 0;
    });
    logTest('5.5 Type filter UI exists', !!typeFilterUI);

    // Test 5.6: Priority filter exists (FIXED V2)
    ...
    // Test 5.7: Status filter exists (FIXED V2)
    const statusFilterUI = Array.from(document.querySelectorAll('*')).find(el => {
      const text = el.textContent?.toLowerCase() || '';
      const hasStatusLabel = text.includes('status');
      const hasStatusOptions = text.includes('active') || text.includes('resolved') || text.includes('archived');
      return hasStatusLabel && hasStatusOptions && el.children.length > 0;
    });
    logTest('5.7 Status filter UI exists', !!statusFilterUI);

    // Test 5.8: View mode filter exists (FIXED V2)
    const viewModeFilterUI = Array.from(document.querySelectorAll('*')).find(el => {
      const text = el.textContent?.toLowerCase() || '';
      const hasViewLabel = text.includes('view') || text.includes('scope');
      const hasViewOptions = text.includes('all') || text.includes('project');
      return hasViewLabel && hasViewOptions && el.children.length > 0;
    });
