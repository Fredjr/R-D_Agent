...
export async function GET(
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/articles/${pmid}/earlier?limit=${limit}`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        return NextResponse.json({
          ...
          message: "Earlier work discovery not yet implemented"
        });
      }
    ...
    console.error('Earlier articles proxy error:', error);
    return NextResponse.json(
