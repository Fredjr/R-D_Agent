...
export async function GET(
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/articles/${pmid}/later?limit=${limit}`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        return NextResponse.json({
          ...
          message: "Later work discovery not yet implemented"
        });
      }
    ...
    console.error('Later articles proxy error:', error);
    return NextResponse.json(
