      ...
      {filteredProtocols.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredProtocols.map(protocol => {
            // Use EnhancedProtocolCard for context-aware protocols
            if (protocol.context_aware) {
              return (
                <EnhancedProtocolCard
                  key={protocol.protocol_id}
                  protocol={protocol}
                  onViewDetails={() => setSelectedProtocol(protocol)}
                  onDelete={() => handleDeleteProtocol(protocol.protocol_id)}
                />
              );
            }

            // Use basic card for legacy protocols
            return (
              <div
                key={protocol.protocol_id}
